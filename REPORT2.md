...
Trained EfficientNet-B0 on balanced ACNE04 patches (val acc 98.7%). On DermNet (4,002 images, 7.8% acne), the unadapted model achieved AUROC = 0.448 — worse than random (predictions inverted). Full domain adaptation (Reinhard color normalization + 2-stage few-shot fine-tuning on 20 DermNet samples + threshold optimization + 6-crop TTA) brought AUROC to 0.665. FaceNet VGGFace2 with just head fine-tuning reached AUROC = 0.569, confirming face-specific pretraining helps. F1 scores remain low (~0.13–0.15) throughout due to 92.2% class imbalance — AUROC is the only reliable metric here.
...
**Positive patches (acne):** Crop each bounding box annotation from the ACNE04 source image, resize to 224×224. Boxes smaller than 20px on either side are skipped — too few pixels to contain meaningful lesion signal (2,304 skipped in train).
...
| No adaptation | EfficientNet-B0 direct | 0.3136 | 0.1293 | 0.4479 |
| Flip score† | Use 1 − prob as acne score | 0.6864 | 0.1469 | 0.5521 |
| Color norm only | Reinhard + histo-match | 0.6619 | 0.1254 | 0.4955 |
| **Full pipeline** | Color norm + 2-stage FT + thresh (0.07) + TTA | 0.1552 | 0.1516 | **0.6651** |
| FaceNet VGGFace2 | Face-pretrained backbone + color norm + thresh (0.44) | 0.4585 | 0.1545 | 0.5690 |

*† Flip score requires knowing inversion exists — detectable with ≥1 labeled DermNet sample. No retraining needed.*

**Reading the table:** Low accuracy on the full pipeline (15.5%) is not a failure — it's the intended behavior of threshold=0.07, which aggressively calls acne to maximize recall on a 7.8%-prevalence positive class. AUROC is threshold-independent and is the true measure of model quality.
...
- Threshold swept 0.05–0.95 on the same 20 samples; 0.07 maximizes F1
- 6-crop TTA: FiveCrop(224) + center horizontal flip, average 6 forward passes

**FaceNet insight:** Frozen VGGFace2 backbone + head fine-tuned on 20 samples = AUROC 0.569, no TTA. Face-specific pretraining provides more transferable skin features than ImageNet alone, with far less engineering.
...
*Left column: baseline (no adaptation). Right column: full pipeline (color norm + 2-stage FT + TTA + threshold=0.07). Top: confusion matrices. Bottom: ROC curves.*

**Confusion matrix interpretation:**

| | Baseline | Full Pipeline |
|---|---|---|
| True Positives (acne caught) | 204 / 312 (65%) | higher recall, lower precision |
| False Positives | 2,639 non-acne wrongly flagged | aggressive at threshold=0.07 |
| AUROC | 0.4479 (below random) | **0.6651** |

The baseline ROC curve dips below the diagonal — a textbook inverted classifier. The full pipeline's ROC curve clears the diagonal across all thresholds, showing genuine discrimination learned from 20 labeled samples.
...
*10 DermNet test images through the fine-tuned EfficientNet-B0. Row 1: 5 acne samples. Row 2: 5 non-acne samples. Green title = correct prediction, red = wrong. At threshold=0.07, the model aggressively calls acne — most non-acne samples are flagged (red), expected at 7.8% class prevalence.*
...
- **Why so many red titles:** Threshold=0.07 means almost any image with weak acne-like signal is flagged. At 92% non-acne prevalence, this floods the non-acne row with false alarms. This is the precision/recall tradeoff made explicit.
...
**Few-shot fine-tuning was the biggest lever.** The 22-point AUROC jump (0.496 → 0.665) came almost entirely from 20 labeled DermNet samples and 2-stage fine-tuning — not from preprocessing. Color normalization fixed calibration but not discrimination. This confirms that feature-level adaptation requires at least some target-domain supervision, even a tiny amount.

**Score flipping was the cheapest win.** AUROC 0.448 → 0.552 for free, once you know the inversion exists. In a real deployment this would be detectable immediately from a small labeled validation set.

**FaceNet surprised.** Frozen VGGFace2 + linear head + color norm = AUROC 0.569, simpler than the full EfficientNet pipeline. For applications with more labeled target data, scaling this approach would likely outperform EfficientNet fine-tuning.

### What Didn't Work

**Color normalization alone is not enough.** AUROC only moved from 0.448 → 0.496 despite visually obvious color transfer. The inversion is a feature-level problem, not a color-statistics problem.
...
Best AUROC achieved: 0.665. Clinical-grade screening typically requires AUROC ≥ 0.85. The remaining gap has three main sources:
